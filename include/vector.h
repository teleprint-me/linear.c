 ...
 * @param type The enumerable type representing the integral type
 */
 ...
 *        generator.
 *
 * This function serves as a helper to initialize a vector's data using a given
 * random number generation method from the Lehmer family of algorithms.
 * It iterates over the vector's elements and populates them with values
 * generated by the provided callback function.
 *
...
/**
 * @brief Initialize a vector using the Lehmer modulo method.
 *
 * The modulo method ($modulo(z) = a \cdot z \mod m$) is the base algorithm of
 * the Lehmer family, generating a sequence of pseudorandom numbers by
 * repeatedly multiplying and taking the modulo. It is deterministic and simple
 * to implement, making it suitable for cases where reproducibility is a key
 * requirement.
 *
 * @param state A pointer to the Lehmer state used for random number
 *              generation.
...
/**
 * @brief Initialize a vector using the Lehmer-Park gamma method.
 *
 * The gamma method ($\gamma(z) = a \cdot (z \mod q) - r \cdot (z \div q)$) is
 * a modified variant of the Lehmer algorithm, producing bounded results within
 * a specific range. The gamma variant introduces additional calculations
 * involving divisions and multiplications, which can create a more diverse
 ...
 * @param vector A pointer to the vector to initialize.
 *
 * @note The gamma method ensures that $| \gamma(z) | \leq m-1$.
 */
void vector_lehmer_gamma(lehmer_state_t* state, vector_t* vector);

/**
 * @brief Initialize a vector using the Lehmer-Park delta method.
 *
 * The delta method ($\delta(z) = (z \div q) - (a \cdot z \div m)$) is another
 * variant of the Lehmer algorithm that produces a binary result (0 or 1).
 * It can be useful for specific types of initialization where a binary
 * or threshold-based sequence is needed.
 *
 * @param state A pointer to the Lehmer state used for random number
 *              generation.
 * @param vector A pointer to the vector to initialize.
 *
 * @note The delta method computes a binary outcome, making it distinct
 *       from the modulo and gamma methods, which produce continuous values.
 */
